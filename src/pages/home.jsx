...
const Home = () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const mappedReports = data.map(r => {
          // Initialize with default values from the raw report record
          let parsed = { 
          ...
          try {
            // Attempt to parse the content string which is stored as JSON in the backend
            const json = JSON.parse(r.content);
            if (json) {
              parsed.title = json.title || parsed.title;
              parsed.category = json.category || parsed.category;
              parsed.description = json.description || json.content || parsed.description;
              parsed.family_members_affected = json.family_members_affected || '';
            }
          } catch (e) {
            // If parsing fails, use the raw content as description
            parsed.description = r.content;
          }
          ...
          return {
            ...
            content: parsed.description + (parsed.family_members_affected ? ` (Family Members Affected: ${parsed.family_members_affected})` : ''),
        ...
        // Sort to show the newest reports first
        setReports(mappedReports.sort((a, b) => new Date(b.created_at) - new Date(a.created_at)));
      }
    } catch (error) {
  ...
  const redIcon = new L.Icon({
    iconUrl: "https://www.clker.com/cliparts/k/a/2/B/c/u/map-marker-red-md.png",
  ...
  const orangeIcon = new L.Icon({
    iconUrl: "https://www.clker.com/cliparts/e/i/N/V/q/J/map-marker-soft-orange-2-md.png",
    shadowUrl: "https://cdnjs.cloudflare.com/ajax/libs/leaflet/0.7.7/images/marker-shadow.png",
    iconSize: [25, 41],
    iconAnchor: [12, 41],
    popupAnchor: [1, -34],
  ...
  return (
    <div className="home-container">
        ...
        <div className="home-modal-overlay">
          <div className="home-modal-card">
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '20px', justifyContent: 'flex-end' }}>
              <button onClick={() => setShowSOSConfirm(false)} className="home-btn-cancel">Cancel</button>
              <button onClick={confirmSOS} className="home-btn-submit" style={{ backgroundColor: '#ef4444', borderColor: '#ef4444' }}>CONFIRM SOS</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="home-sidenav">
        ...
        <button type="button" className="home-sos-btn" onClick={handleSOS}>
        ...
        <button type="button" className="home-logout-btn" onClick={handleLogout}>
          <svg className="home-tab-icon" width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <main className="home-main-content">
        ...
        {(currentHash === '#report') && (
          <section id="report" className="home-section">
                ...
                <div className="home-modal-card">
                  ...
                  <form onSubmit={handleAddItem}>
                    <div className="home-form-group">
                      <select value={newItem.category} onChange={(e) => setNewItem({...newItem, category: e.target.value})} className="home-form-select">
                    ...
                    <div className="home-form-group">
                      <input type="text" placeholder="Household Name" value={newItem.title} onChange={(e) => setNewItem({...newItem, title: e.target.value})} required className="home-form-input" />
                    </div>
                    <div className="home-form-group">
                      <input type="number" placeholder="Number of Family Members Affected" value={newItem.family_members_affected} onChange={(e) => setNewItem({...newItem, family_members_affected: e.target.value})} className="home-form-input" min="1" />
                    </div>
                    
                    <div className="home-form-group">
                      <label style={{display:'block', marginBottom:'5px', color:'#000'}}>Location (Drag marker to adjust):</label>
                      {!userLocation ? (
                        ...
                        <MapContainer 
                          ...
                          zoom={15} 
                          style={{ height: '250px', width: '100%', borderRadius: '8px', marginBottom: '10px' }}
                        >
                          ...
                          <Marker 
                            ...
                            eventHandlers={{
                              dragend: (e) => {
                                ...
                                setNewItem(prev => ({...prev, latitude: position.lat, longitude: position.lng}));
                              },
                            }}
                          />
                        </MapContainer>
                    ...
                    <div className="home-form-group">
                      ...
                      <input type="datetime-local" value={newItem.report_date} onChange={(e) => setNewItem({...newItem, report_date: e.target.value})} className="home-form-input" />
                    </div>
                    <div className="home-form-group">
                      <label style={{display:'block', marginBottom:'5px', color:'#000'}}>Attachments (Max 3):</label>
                      <input type="file" multiple onChange={handleFileChange} className="home-form-input" />
                    </div>
                    <div className="home-form-group">
                      <textarea placeholder="Details..." value={newItem.content} onChange={(e) => setNewItem({...newItem, content: e.target.value})} required className="home-form-textarea" />
                    </div>
            ...
            <div className="home-fab-container">
              <button type="button" className="home-fab" onClick={() => setShowForm(true)}>+</button>
            </div>
          </section>
        )}
            ...
            <div className="home-overview-grid">
              {announcements.map(item => (
                <div key={item.id} className="home-card">
                  ...
                  <div className="home-card-dash" aria-hidden="true" />
                  <p className="home-card-text">{item.content}</p>
                  <small style={{ display: 'block', marginTop: '10px' }}>Posted: {new Date(item.created_at).toLocaleString()}</small>
                </div>
              ))}
            </div>
          </section>
        ...
        {currentHash === '#emap' && (
          <section id="emap" className="home-section">
            ...
            {!userLocation ? (
              ...
              >
                <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
                {reports.map(item => item.latitude && (
                  <Marker key={item.id} position={[item.latitude, item.longitude]} icon={orangeIcon}>
                    <Popup><strong>{item.title}</strong><br/>{item.content}</Popup>
                  </Marker>
                ))}
                {sosRequests.map(sos => sos.latitude && (
                  <Marker key={sos.id} position={[sos.latitude, sos.longitude]} icon={redIcon}>
                    <Popup><strong>SOS Request</strong><br/>Sent At: {new Date(sos.created_at).toLocaleString()}</Popup>
                  </Marker>
                ))}
                {polygonZones.map(poly => poly.polygon_data?.geometry && (
                  <Polygon 
                    key={poly.id} 
                    positions={poly.polygon_data.geometry.coordinates[0].map(c => [c[1], c[0]])}
                    pathOptions={{ 
